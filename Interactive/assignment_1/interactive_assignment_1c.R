...
diff_df <- left_join(libor, treas_3m) %>%
  mutate(diff = libor_rate - treas_3m_rate) %>%
  select(c(date, diff)) %>%
  drop_na()
...
graph <- ggplot() +
  ...
  geom_line(data = diff_df, aes(x = date, y = diff), color = "black") +
  geom_hline(yintercept = 0) +
  theme_classic() + 
  labs(x = "Year", y = "%", title = "Interest Rates",
  ...
  annotate("text", x = as.Date("2018-01-01"), y = .1, label = "Spread")
